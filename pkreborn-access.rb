...
class Game_Player < Game_Character
	...
	def searchEvent()
		...
		#  searchableEvents = loadSearchEvents
		searchableEvents = [SearchEvent.new("Last selected category", nil, nil, 0),
												...
												SearchEvent.new("Clickable event", "completeMap", "0", 4), #Search clickable events in all map instead
		...
		input = $game_variables[5]
		if input == -1 #Abort input
			return
		elsif input == 0 #"Last selected category" was selected
			if @@lastSelectedSearchItem == -1 #No prior category was selected
		...
		if input == 1 || input == 2 || input == 3 || input == 4
			...
			when 1, 2 #Healing Spot & Merchant
				...
				for mwep in mapsWithEntryPoint
					amountSearchResults = amountSearchResults + mwep.eventArray.length
				end
				Kernel.pbMessage((eventsArray.length + amountSearchResults).to_s + ' ' + searchableEvents[input].name.to_s + " detected.")
				if (eventsArray.length + amountSearchResults) == 0 #No event
					#Do nothing
				elsif (eventsArray.length + amountSearchResults) == 1 #exact one event
					if eventsArray.length == 1 #Event is on the same map as the player
						...
						if route.length == 0
							...
							if direction != 0
								addAdjacentNode(route, direction)
							end
							printInstruction(convertRouteToInstructions(route))
						end
					else
						#Event is not on the same map as the player
						firstRoute = aStern(Node.new(@x, @y), Node.new(mapsWithEntryPoint[0].event.x, mapsWithEntryPoint[0].event.y))
						secondRoute = aStern(Node.new(mapsWithEntryPoint[0].x, mapsWithEntryPoint[0].y), Node.new(mapsWithEntryPoint[0].eventArray[0].x, mapsWithEntryPoint[0].eventArray[0].y), mapsWithEntryPoint[0].map)
						direction = 0
						if secondRoute.length == 0
							possibleTargets = getSaleTilesOfNPC(mapsWithEntryPoint[0].eventArray[0], mapsWithEntryPoint[0].map)
							for pt in possibleTargets
								secondRoute = aStern(Node.new(mapsWithEntryPoint[0].x, mapsWithEntryPoint[0].y), Node.new(pt.node.x, pt.node.y), mapsWithEntryPoint[0].map)
								if secondRoute.length != 0
									direction = pt.direction
									break;
								end
				...
				elsif (eventsArray.length + amountSearchResults) > 1 #multiple Events are on this map or another map
					s = "5,-1"
					for event in eventsArray
						s = s + "," + $game_map.name + " (" + event.x.to_s + "; " + event.y.to_s + ")"
					end
					for mwep in mapsWithEntryPoint
						for event in mwep.eventArray
							s = s + "," + mwep.map.name + " (" + event.x.to_s + "; " + event.y.to_s + ")"
						end
					end
					Kernel.pbMessage("Please select a destination. Your current location is " + $game_map.name + ".\\ch[" + s + "]")
					input2 = $game_variables[5]
					#Evaluation of input (multiple targets)
					if input2 == -1
						#do nothing
						...
						input2 = input2 - eventsArray.length
						inputMwep = nil
						#          Kernel.pbMessage(input2.to_s)
						for mwep in mapsWithEntryPoint
							#         Kernel.pbMessage(mwep.eventArray.length.to_s)
							if input2 < mwep.eventArray.length
								inputMwep = mwep
								break
							end
							input2 = input2 - mwep.eventArray.length
						end
						#Kernel.pbMessage((inputMwep == nil).to_s)
						if inputMwep == nil
							#do nothing
						else
							firstRoute = aStern(Node.new(@x, @y), Node.new(inputMwep.event.x, inputMwep.event.y))
							secondRoute = aStern(Node.new(inputMwep.x, inputMwep.y), Node.new(inputMwep.eventArray[input2].x, inputMwep.eventArray[input2].y), inputMwep.map)
							direction = 0
							if secondRoute.length == 0
								possibleTargets = getSaleTilesOfNPC(inputMwep.eventArray[input2], inputMwep.map)
								for pt in possibleTargets
									secondRoute = aStern(Node.new(inputMwep.x, inputMwep.y), Node.new(pt.node.x, pt.node.y), inputMwep.map)
									if secondRoute.length != 0
										direction = pt.direction
										break;
									end
			...
			when 3 #Teleport Tiles
				for event in eventsArray
					if aStern(Node.new(@x, @y), Node.new(event.x, event.y)).length == 0
						eventsArray.delete(event)
					end
				end
				Kernel.pbMessage(eventsArray.length.to_s + ' ' + searchableEvents[input].name.to_s + " detected.")
				if eventsArray.length == 0
					#Do nothing
				elsif eventsArray.length == 1 #One Event Found
					...
					for event in eventsArray
						...
						s = s + "," + teleportDestination + " (" + event.x.to_s + "; " + event.y.to_s + ")"
					end
					Kernel.pbMessage("Please select a destination. Your current location is " + $game_map.name + ".\\ch[" + s + "]")
					input2 = $game_variables[5]
					if input2 == -1
						#Do nothing
					elsif input2 == 0 #Nearest event
						printInstruction(convertRouteToInstructions(searchToNearestEvent(eventsArray)))
					else
						#Nearest event Method is used with an array with a single element (for my convenience)
						printInstruction(convertRouteToInstructions(searchToNearestEvent(eventsArray[input2 - 1, 1])))
					end
				end

			when 4 #Clickable Events
				#TODO händler oder heilung über theke / counter
				Kernel.pbMessage(eventsArray.length.to_s + ' ' + searchableEvents[input].name.to_s + " detected.")
				if eventsArray.length == 0
					#Do nothing
				elsif eventsArray.length == 1 #One Event Found
